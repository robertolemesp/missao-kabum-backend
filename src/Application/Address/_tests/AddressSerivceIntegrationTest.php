...
class AddressServiceIntegrationTest extends TestCase {
  ...
  protected function setUp(): void {
    ...
    $this->addressRepository = new MySQLAddressRepository($this->dbConnection);
  ...
  public function testCreateAddress() {
    ...
    $this->assertEquals('1', $retrievedAddresses[0]->getNumber());
  ...
  public function testUpdateAddress() {
    ...
    $addresses = [
      new Address(null, $customerId, 'Rua Durval Clemente', '1', '12345-678')
    ];
    ...
    $this->assertEquals('2', $retrievedAddresses[0]->getNumber());
    $this->assertEquals('02041-000', $retrievedAddresses[0]->getZipcode());
  }
  
  public function testRemoveAddress() {
    $customerService = $this->container->getCustomerService();
    ...
    $addressService->createMany($customerId, $addresses);
  
    $retrievedAddresses = $addressService->listByCustomerId($customerId);
    $this->assertNotEmpty($retrievedAddresses);
  
    $addressIds = array_map(fn($address) => $address->getId(), $retrievedAddresses);
  
    $addressService->removeMany($addressIds);
  ...
  public function testAddressValidation() {
    ...
    $invalidAddress = new Address(null, 1, '', '1', '12345-678');
  }
  ...
  public function testInvalidZipcodeFormat() {
    ...
    $invalidAddress = new Address(null, 1, 'Rua Teste', '1', '1234');
  }
}
